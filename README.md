...
| 模块 | 说明 |
| :--- | :--- |
| 书库管理 | 多书库（书库名 slug 唯一、独立主题色/固定标签），EPUB 以 `[系列拼音-N] 作者拼音-书名拼音.epub` 统一命名；**封面缺失自动恢复**（启动时扫描，按 本地缓存 → EPUB 内嵌 → 远程来源 依次找回，设置页可手动触发） |
...
| 数据统计 | LLM 用量按服务商/模型累计，按价格表实时换算成本（元），支持花费预算自动熔断 |
